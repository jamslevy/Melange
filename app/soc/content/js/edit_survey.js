    ...
    $("#new_item_dialog").dialog({
      ...
      buttons: {
        'Add option': function() {
          ...
          $('#' + ul_id).append('<li id="id-li-' + ul_id + '_' + i +
      ...
      'Add question': function() {
        ...
        if (field_name != '') {
          ...
          if (new_field) {
            ...
            formatted_name = SURVEY_PREFIX + new_field_count + type +  field_name.replace(/\s/g, '-_-');
            if (button_id == 'choice')  {
...
$(this).find("#id_survey_html").attr('value', widget.html());
  ...
  survey.find('.sortable').each(function() {
    $('#order_for_' + this.id).val($('#' + this.id).sortable('serialize'));
  });
